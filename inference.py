...
class ToyDomainDiffusion(nn.Module):
    def __init__(self, device):
        ...
        for id, model in enumerate(self.models):
            model.load_state_dict(torch.load(f"models/simple_diffusion_model_{id+1}.pth", map_location=device))
            model.to(device)
            model.eval()

        self.views = [(0, 2), (1, 3), (2, 4)]  # Each model covers two modes
    ...
    def gsc(self, batch_size=100, num_inference_steps=100, device='cuda'):
        ...
        latent = torch.randn((batch_size, 4)).to(device) 
...
if __name__ == '__main__':
    ...
    plt.scatter(np.ones_like(latents[:, 0]) * 0., latents[:, 0], label='x1', alpha=0.5)
    plt.scatter(np.ones_like(latents[:, 1]) * 1., latents[:, 1], label='x2', alpha=0.5)
    plt.scatter(np.ones_like(latents[:, 2]) * 2., latents[:, 2], label='x3', alpha=0.5)
    plt.scatter(np.ones_like(latents[:, 3]) * 3., latents[:, 3], label='x4', alpha=0.5)
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f"results/{opt.outfile}_samples_{opt.mode}_{opt.steps}_steps.png")
    plt.close()
