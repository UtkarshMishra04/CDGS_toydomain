...
class MLPBlock(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hidden_dim: int, time_dim: int, dropout: float = 0.1
    ):
        super().__init__()
...
class Simple2DUNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, timestep: torch.Tensor) -> torch.Tensor:
        ...
        for up, blocks, skip in zip(
            self.decoder_ups, self.decoder_blocks, reversed(skip_connections)
        ):
            x = F.silu(up(x))
...
class SimpleDiffusionModel(nn.Module):
    ...
    def __init__(
        self, unet: Optional[nn.Module] = None, input_dim: int = 2, **unet_kwargs
    ):
        super().__init__()
        self.input_dim = input_dim
...
class FlowMatchingModel(nn.Module):
    ...
    @torch.no_grad()
    def sample(
        self, batch_size: int, device: torch.device, num_inference_steps: int = 100
    ) -> torch.Tensor:
        """
