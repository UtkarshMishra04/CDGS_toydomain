...
class MultiModalDataset(Dataset):
    ...
    def _generate_data(self, num_samples: int) -> None:
        ...
        x1 = np.array(self.start_means)[self.start_labels] + self.rng.normal(
            loc=0, scale=np.array(self.start_stds)[self.start_labels]
        )
        x2 = np.array(self.end_means)[self.end_labels] + self.rng.normal(
            loc=0, scale=np.array(self.end_stds)[self.end_labels]
        )
    ...
    def _plot_dataset_samples(
        self, ax: plt.Axes, n: int, annotate_valid: bool, num_stds: float
    ) -> None:
            ...
            for start_val, end_val in zip(start_vals, end_vals):
                if annotate_valid:
                    is_valid = self._is_valid(
                        start_val, end_val, mode_x1, mode_x2, num_stds
                    )
    ...
    def plot_transitions(
        ...
        self._plot_distributions(
            ax, 0, self.start_means, self.start_stds, y_range, "Start"
        )
        self._plot_distributions(
            ax, 1, self.end_means, self.end_stds, y_range, "End", len(self.start_means)
        )
    ...
    def _is_valid(
        ...
        start_valid = (
            abs(start_val - self.start_means[start_mode])
            <= num_stds * self.start_stds[start_mode]
        )
        end_valid = (
            abs(end_val - self.end_means[end_mode])
            <= num_stds * self.end_stds[end_mode]
        )
    ...
    def _validate_transition_step(
        ...
        for mode1_idx, (mean1, std1) in enumerate(
            zip(dataset1.end_means, dataset1.end_stds)
        ):
            for mode2_idx, (mean2, std2) in enumerate(
                zip(dataset2.start_means, dataset2.start_stds)
            ):
                # Check if this transition is allowed in both datasets
                if hasattr(dataset1, "transition_matrix") and hasattr(
                    dataset2, "transition_matrix"
                ):
    ...
    def _validate_full_path(
        path: np.ndarray, datasets: List["MultiModalDataset"], num_stds: float = 3.0
        ...
        for i in range(len(datasets)):
            ...
            for mode_x1, mode_x2 in zip(*np.where(datasets[i].transition_matrix)):
                start_valid = (
                    abs(path[i] - datasets[i].start_means[mode_x1])
                    <= num_stds * datasets[i].start_stds[mode_x1]
                )
                end_valid = (
                    abs(path[i + 1] - datasets[i].end_means[mode_x2])
                    <= num_stds * datasets[i].end_stds[mode_x2]
                )
    ...
    def evaluate_paths(
        ...
        """
        results = [
            MultiModalDataset._validate_full_path(path, datasets, num_stds)
            for path in paths
        ]
    ...
    def plot_multi_step_transitions(
        ...
        y_range = (
            min(sample_min, mean_min - 3 * std_max) - 0.5,
        ...
        MultiModalDataset._plot_distributions_static(
            ax, 0, datasets[0].start_means, datasets[0].start_stds, y_range, "Step 0", 0
        )
    ...
    def compute_accuracy(self, samples: np.ndarray, num_stds: float = 3.0) -> float:
        ...
        for start_val, end_val in samples:
            ...
            for mode_x1, mode_x2 in zip(*np.where(self.transition_matrix)):
                start_valid = (
                    abs(start_val - self.start_means[mode_x1])
                    <= num_stds * self.start_stds[mode_x1]
                )
                end_valid = (
                    abs(end_val - self.end_means[mode_x2])
                    <= num_stds * self.end_stds[mode_x2]
                )
...
if __name__ == "__main__":
    ...
    fig.savefig(
        "figures_notebook/custom_transition_matrix_dataset.png",
        dpi=300,
        bbox_inches="tight",
    )
    ...
    for i in range(n_samples):
        if i < 25:  # First half: valid-ish samples
            multi_step_samples[i, 0] = np.random.choice([0, 2]) + np.random.normal(
                0, 0.3
            )
            multi_step_samples[i, 1] = np.random.choice([1, 3]) + np.random.normal(
                0, 0.3
            )
            multi_step_samples[i, 2] = np.random.choice([0.5, 2.5]) + np.random.normal(
                0, 0.3
            )
    ...
    fig_multi.savefig(
        "figures_notebook/multi_step_transitions_test.png", dpi=300, bbox_inches="tight"
    )
